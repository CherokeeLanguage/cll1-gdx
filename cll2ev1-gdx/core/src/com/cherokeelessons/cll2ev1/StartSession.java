...
public class StartSession implements Runnable {
	...
	public void run() {
		...
		/*
		 * Always create a fresh deck by copying (cloning) cards from the
		 * CSV master deck. If we have stats that don't match a card in the
		 * fresh deck they will be ignored and lost at next save. The match
		 * up is done by "card id" which is the minimal amount of uniqueness
		 * to match up as generated by the CardData object.
		 */
		Deck<CardData> masterDeck = new Deck<CardData>();
		for (GameCard card : ((CLL2EV1) game).cards) {
			masterDeck.add(card.copy());
		}
		Deck<CardData> activeDeck = new Deck<CardData>();
		/*
		 * The json card stats data aren't stored directly as a single json
		 * object in a list to reduce memory requirements for serialization
		 * and deserialization. Each valid set of stats indicates a card
		 * already "in play". Update each such card with the saved stats and
		 * move each one into the "active deck".
		 */
		for (String jsonCardStats : jsonCardsStats) {
			...
			copyStatsLoop: for (ICard<CardData> card : masterDeck.getCards()) {
				...
				// move this "in play" card into the active deck.
				activeDeck.add(card);
				break copyStatsLoop;
			}
		}
